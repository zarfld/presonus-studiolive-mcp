    ...
    server.tool(
      ...
      async ({ deviceId, channelId, compressor, gate, limiter }) => {
        ...
        if (compressor) {
          ...
          if (compressor.attackMs !== undefined) changes.push({ parameter: 'comp.attack', rawKeyPath: `${channelId}.comp.attack`, currentRawValue: typeof f('.comp.attack') === 'number' ? f('.comp.attack') as number : null, proposedRawValue: compAttackMsToNormalized(compressor.attackMs), currentDisplayValue: typeof f('.comp.attack') === 'number' ? `${normalizedToAttackMs(f('.comp.attack') as number).toFixed(0)} ms` : '(unknown)', proposedDisplayValue: `${compressor.attackMs.toFixed(0)} ms` })
          if (compressor.releaseMs !== undefined) changes.push({ parameter: 'comp.release', rawKeyPath: `${channelId}.comp.release`, currentRawValue: typeof f('.comp.release') === 'number' ? f('.comp.release') as number : null, proposedRawValue: compReleaseMsToNormalized(compressor.releaseMs), currentDisplayValue: typeof f('.comp.release') === 'number' ? `${normalizedToReleaseMs(f('.comp.release') as number).toFixed(0)} ms` : '(unknown)', proposedDisplayValue: `${compressor.releaseMs.toFixed(0)} ms` })
        }
        if (gate) {
          ...
          if (gate.attackMs !== undefined) changes.push({ parameter: 'gate.attack', rawKeyPath: `${channelId}.gate.attack`, currentRawValue: typeof f('.gate.attack') === 'number' ? f('.gate.attack') as number : null, proposedRawValue: gateAttackMsToNormalized(gate.attackMs), currentDisplayValue: typeof f('.gate.attack') === 'number' ? `${normalizedToAttackMs(f('.gate.attack') as number).toFixed(0)} ms` : '(unknown)', proposedDisplayValue: `${gate.attackMs.toFixed(0)} ms` })
          if (gate.releaseMs !== undefined) changes.push({ parameter: 'gate.release', rawKeyPath: `${channelId}.gate.release`, currentRawValue: typeof f('.gate.release') === 'number' ? f('.gate.release') as number : null, proposedRawValue: gateReleaseMsToNormalized(gate.releaseMs), currentDisplayValue: typeof f('.gate.release') === 'number' ? `${normalizedToReleaseMs(f('.gate.release') as number).toFixed(0)} ms` : '(unknown)', proposedDisplayValue: `${gate.releaseMs.toFixed(0)} ms` })
        ...
        if (limiter) {
          ...
          if (limiter.releaseMs !== undefined) changes.push({ parameter: 'limiter.release', rawKeyPath: `${channelId}.limit.release`, currentRawValue: typeof f('.limit.release') === 'number' ? f('.limit.release') as number : null, proposedRawValue: compReleaseMsToNormalized(limiter.releaseMs), currentDisplayValue: typeof f('.limit.release') === 'number' ? `${normalizedToReleaseMs(f('.limit.release') as number).toFixed(0)} ms` : '(unknown)', proposedDisplayValue: `${limiter.releaseMs.toFixed(0)} ms` })
        }
