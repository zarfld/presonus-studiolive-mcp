...
- [Packages](#packages)
- [MCP server â€” tools and resources](#mcp-server--tools-and-resources)
- [Probe CLI](#probe-cli)
...
This is a pnpm monorepo with four packages under `packages/`. Dependencies flow one way: `domain` â† `adapter` â† `inspector` | `server`.

### `@presonus-mcp/domain`

Pure TypeScript/Zod schema library â€” the single source of truth for all data contracts.

- `MixerIdentitySchema` â€” deviceId, serial, IP, port, role, controllable
- `MixerChannelSchema` â€” name, mute, solo, fader (dB/linear/raw), pan, color, `compModelName`, `eqModelName`
- `MeterSummarySchema` â€” time-windowed channel activity classification
- `FatCompressorStateSchema` / `FatEqStateSchema` â€” discriminated unions per model
- `COMPRESSOR_MODEL_BY_CLASSID` / `EQ_MODEL_BY_CLASSID` â€” full GUID â†’ model name maps for scene file decoding
- `decodeCompressorModel(value)` / `decodeEqModel(value)` â€” live state float â†’ model name helpers
...
- `discoverMixers({ timeoutMs? })` â€” UDP broadcast discovery
- `PresonusClientManager` â€” connects/disconnects clients; provides `getSnapshot()`, `getIdentity()`, `getSummarizer()`
- `flattenFeatherbearState()` â€” converts featherbear's nested `_data.internal.children.*` tree to flat dot-notation keys
...
## MCP server â€” tools and resources

### Tools

The current tool and resource inventory is generated from source â€” see **[docs/capability-matrix.generated.md](docs/capability-matrix.generated.md)**.

Write tools are registered only when `writeEnabled: true` (default: false). All write tools use the ProposedChangeSet workflow (propose â†’ 60 s TTL â†’ apply) and include a `changeSetConfidence` field so operators know how well-calibrated the proposed values are.
...
// â†’ [{ deviceId, serial, model, firmware, role, ipAddress, port }]
```
...
// â†’ { valid: boolean, reasons: string[] }
```
...
// â†’ { status: "ok"|"warning"|"error", issues[], printablePatchRows[] }
```
...
// â†’ ChannelFatState (eqModel, compModel, eqBands, comp, gate, limiter, hpfFrequencyHz)
```
...
// â†’ { checks: [{ check, passed, detail }], warnings: string[], parameterConfidence }
```
...
```typescript
{
  id:             string,       // "line.ch1", "aux.ch3", "sub.ch1", â€¦
  selector:       { type, channel },
  name?:          string,       // label from mixer (e.g. "Kick In")
  mute?:          boolean,
  solo?:          boolean,
  fader?:         { db: number|null, linear: number|null },
  pan?:           number,       // 0.0 left â€¦ 0.5 center â€¦ 1.0 right
  linked?:        boolean,
  color?:         string,       // RGBA hex

  // Fat Channel â€” decoded from live opt.compmodel / opt.eqmodel
...
Output patch router â€” source index known for each analog/AVB output; `sourceName: null` until probe confirms source â†’ index mapping.
...
Full raw state dump (pre-normalized flat dot-notation). For diagnostics and development only â€” do not use for agent reasoning logic.

> **Layer A / Layer B routing model**: Layer A resources and tools return data directly observable from confirmed state keys. Layer B tools (`get_input_routing`, `validate_avb_routing`) return `not_verifiable_with_current_adapter` and include probe instructions â€” physical cable routing and AVB stream assignments cannot be verified by software alone.
...
| `diff-state --before <f> --after <f>` | Compare two state dumps; identify changed keys (workflow: change one control â†’ dump-state â†’ diff â†’ name the key) |
...
> **Note**: Scene file content (`__classid` GUIDs) is not accessible over the network API â€” it returns 0 bytes. Fat Channel model identity must be read from live state via `opt.compmodel.value` / `opt.eqmodel.value`.
...
- Quick-reference selector tables (source instrument â†’ recommended model)
...
| HIL (hardware-in-loop) | `pnpm test:hil` | Yes â€” see below |
...
| Prefix | Type | Example |
|---|---|---|
| `StR-NNN` | Stakeholder requirement | #1â€“#4 |
| `REQ-F-*` | Functional requirement | #15â€“#46 |
| `REQ-NF-*` | Non-functional requirement | #21â€“#24 |
| `ADR-*` | Architecture decision | #47 |
| `QA-SC-*` | Quality attribute scenario | #25â€“#27, #49â€“#50 |
| `TEST-*` | Verification test case | #51â€“#60, #80â€“#83 |

Issues are organized into milestones v0.1â€“v1.0. See the [GitHub Issues tab](https://github.com/zarfld/presonus-studiolive-mcp/issues) for the full traceability register.

### Adding a new raw state key

1. `pnpm probe:dev dump-state -d <ip>` before and after changing a control in UC Surface
2. `pnpm probe:dev diff-state --before <before.json> --after <after.json>` â€” identifies the changed key
...
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  @presonus-mcp/server                   â”‚
â”‚  See docs/capability-matrix.generated.md â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                 â”‚ internal API
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  @presonus-mcp/adapter                  â”‚
â”‚  PresonusClientManager                  â”‚
â”‚  flattenFeatherbearState()              â”‚
â”‚  PresonusMeterSummarizer                â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                 â”‚ TCP 53000 (UC Surface protocol)
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  StudioLive III mixer                   â”‚
â”‚  (32SC, 32R, 24R, 16R, 16)             â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```

Domain schemas (`@presonus-mcp/domain`) sit outside this stack and are imported by both adapter and server â€” never the other way round.
...
- **Show prep layer** â€” `ShowInputSchema` is a stub (rider analysis, channel template suggestions).
- **Scene file access** â€” `__classid` GUIDs not accessible over network; only live state model IDs available.
- **HIL test coverage** â€” tests with `*.hil.test.ts` require a physical mixer.
